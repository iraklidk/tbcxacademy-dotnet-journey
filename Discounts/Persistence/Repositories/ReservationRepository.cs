...
public class ReservationRepository : BaseRepository<Reservation>, IReservationRepository
{
    ...
    public async Task<List<Reservation>> GetByUserAsync(int userId, CancellationToken ct = default)
        => await _context.Reservations.Include(r => r.Customer).Include(r => r.Offer)
                                          .Where(r => r.Customer.UserId == userId).ToListAsync(ct).ConfigureAwait(false);
}
